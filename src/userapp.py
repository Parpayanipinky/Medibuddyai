...
LAB_PATTERNS = [
    r"([A-Za-z][A-Za-z0-9\s\/\-\(\)%\+]{1,45})\s+([<>]?\s*\d+\.?\d*)\s*([A-Za-z%\/\-\^0-9\.µμ]*)\s+(\d+\.?\d*\s*[-–]\s*\d+\.?\d*)",
    r"([A-Za-z][A-Za-z0-9\s\/\-\(\)%\+]{1,45})\s*[:\-]?\s*([<>]?\s*\d+\.?\d*)\s*([A-Za-z%\/\-\^0-9\.µμ]*)\s*\(?\s*(\d+\.?\d*\s*[-–]\s*\d+\.?\d*)\s*\)?",
]
...
def extract_numeric_with_optional_unit(text):
    ...
    if not raw:
        return "", ""
    raw = raw.replace(",", " ")
    m = re.search(r"([<>]?\d+(?:\.\d+)?)\s*([A-Za-zµμ%/^xX0-9\.-]+)?", raw)
    if not m:
        return "", ""
...
def _ocr_candidate_score(text):
    ...
    units = re.findall(
        r"\b(?:mg/dl|g/dl|gm/dl|mmol/l|u/l|iu/l|x10\^?\d+/?(?:l|ul|µl|ml)?|10\^?\d+/?(?:l|ul|µl|ml)?|%|fl|pg|ng/ml|miu/l|µiu/ml)\b",
        text,
        flags=re.IGNORECASE,
    )
...
def _dynamic_lab_row_score(text):
    ...
    unit_re = r"(?:g/dl|gm/dl|mg/dl|mmol/l|u/l|iu/l|x10\^?\d+/?[a-zµμ]*|10\^?\d+/?[a-zµμ]*|%|fl|pg|ng/ml|miu/l|ratio)"
...
def looks_unit_token(token):
    ...
    if not token:
        return False
    return bool(re.fullmatch(r"[A-Za-zµμ%/\^xX0-9\.-]+", token))
...
def parse_noisy_cbc_records(text):
    ...
    unit_pattern = r"(?:x\s*10\s*E?\s*\d+\s*/?\s*[A-Za-zµμ]*|x10E?\d+/?[A-Za-zµμ]*|x10\^?\d+/?[A-Za-zµμ]*|10\^?\d+/?[A-Za-zµμ]*|g\s*/\s*dL|gm\s*/\s*dL|mg\s*/\s*dL|mmol\s*/\s*L|IU\s*/\s*L|U\s*/\s*L|fL|pg|%|ratio)"
...
def parse_lab_records(text):
    ...
    def parse_known_noisy_lab_line(line):
        ...
        unit_match = re.search(r"(x\s*10\s*e?\d+/?[A-Za-zµμ]*|x10E?\d+/?[A-Za-zµμ]*|10\^?\d+/?[A-Za-zµμ]*|g/dl|gm/dl|mg/dl|mmol/l|iu/l|u/l|fl|pg|%|ratio)", after_value, flags=re.IGNORECASE)
...
def build_validation_error_html(message):
    ...
    <div class="xray-card">
        ...
        <div class="xray-disclaimer-box">⚠️ {html.escape(safe_str(message))}</div>
    </div>
    """
...
def _basic_urdu_translation(text, context="medical report"):
    ...
    if "educational" in lower and ("diagnosis" in lower or "medical advice" in lower):
        return "یہ خلاصہ صرف تعلیمی مقصد کے لیے ہے۔ یہ حتمی طبی تشخیص یا علاج کا مشورہ نہیں ہے۔ براہِ کرم مستند ڈاکٹر سے مشورہ کریں۔"
    if "no patient information" in lower:
        return "اپ لوڈ کی گئی رپورٹ میں مریض کی معلومات واضح طور پر نہیں مل سکیں۔"
    if "x-ray" in lower or "x ray" in lower or "radiology" in lower:
        status_line = ""
        if "needs attention" in lower:
            status_line = " اس تصویر میں کچھ ایسی باتیں نظر آ رہی ہیں جن پر ڈاکٹر یا ریڈیولوجسٹ کی توجہ ضروری ہے۔"
        elif "no obvious acute abnormality" in lower:
            status_line = " اس تصویر میں کوئی واضح فوری غیر معمولی بات نظر نہیں آتی، لیکن حتمی رائے ڈاکٹر دے گا۔"
        elif "limited" in lower or "unclear" in lower:
            status_line = " تصویر مکمل طور پر واضح نہیں، اس لیے ڈاکٹر سے دوبارہ جائزہ لینا بہتر ہے۔"
        return (
            "یہ ایک ایکس رے رپورٹ کا آسان اردو خلاصہ ہے۔" + status_line +
            " رپورٹ میں درج اہم نکات کو آسان الفاظ میں سمجھایا گیا ہے تاکہ صارف کو بنیادی سمجھ آ سکے۔ "
            "یہ حتمی تشخیص نہیں ہے؛ درد، سوجن، چوٹ یا حرکت میں کمی کی صورت میں ڈاکٹر سے فوری مشورہ کریں۔"
        )
    if "laboratory" in lower or "cbc" in lower or "hemoglobin" in lower or "monocytes" in lower or "low" in lower or "high" in lower:
        ...
        if "hemoglobin" in lower:
            parts.append("ہیموگلوبن کی مقدار رپورٹ میں قابلِ توجہ ہے۔")
        if "low" in lower:
            parts.append("کچھ ٹیسٹ نارمل حد سے کم ہیں۔")
        if "high" in lower:
            parts.append("کچھ ٹیسٹ نارمل حد سے زیادہ ہیں۔")
        if not parts:
            parts.append("یہ لیبارٹری رپورٹ کے نتائج کا آسان خلاصہ ہے۔")
        return " ".join(parts) + " ان نتائج کا مطلب سمجھنے کے لیے اپنی مکمل علامات اور طبی تاریخ کے ساتھ ڈاکٹر سے مشورہ کریں۔ یہ معلومات صرف تعلیمی مقصد کے لیے ہیں۔"
    ...
    value_note = "\n".join(values)
    if value_note:
        return "رپورٹ میں درج چند اہم قدریں نیچے دی گئی ہیں۔ براہِ کرم ان کی تشریح کے لیے ڈاکٹر سے مشورہ کریں:\n" + value_note
    return "یہ رپورٹ کا آسان اردو تعلیمی خلاصہ ہے۔ براہِ کرم اسے حتمی تشخیص نہ سمجھیں اور مستند ڈاکٹر سے مشورہ کریں۔"
...
def save_pdf_report(payload, language_choice="English"):
    ...
    def add_urdu_report():
        ...
        UH("اے آئی میڈیکل رپورٹ خلاصہ")
        U("تعلیمی خلاصہ فقط · یہ حتمی تشخیص نہیں ہے")
        UH("رپورٹ کی معلومات")
        U(f"رپورٹ کیٹیگری: {report_category}\nرپورٹ ٹائپ: {report_subtype}")
        if patient_info:
            UH("مریض کی معلومات")
        ...
        U(urdu_explanation)
        UH("اہم نوٹ")
        U("یہ رپورٹ صرف تعلیمی مقصد کے لیے ہے۔ اسے حتمی طبی تشخیص یا علاج کا مشورہ نہ سمجھیں۔ براہِ کرم مستند ڈاکٹر سے مشورہ کریں۔")
...
def build_xray_visual_html(result):
    ...
    <div class="xray-card">
        ...
        <div class="xray-disclaimer-box">⚠️ {html.escape(result["caution"])}</div>
    </div>
    """
...
def ask_question_about_report(question, chat_history):
    ...
    if not CURRENT_REPORT_CONTEXT.get("formatted_text") and not CURRENT_REPORT_CONTEXT.get("ai_explanation"):
        chat_history = chat_history + [
            ...
            {"role": "assistant", "content": "⚠️ Please analyze a lab report or X-ray first. Then I can answer your questions using your actual report data."}
        ]
        return chat_history, ""
    ...
    # Abnormal / table / what's wrong
        ...
        fallback_answer = (
            f"### ⚠️ Values Needing Attention ({len(abnormal_records)} found)\n\n"
...
def build_xray_personalized_suggestions(result):
    ...
    if _is_chest_body_part(body_part):
        ...
        suggestions = [
            ...
            "For pain or swelling, use a cold pack wrapped in cloth for 15–20 minutes at a time, especially during the first 24–48 hours.",
            ...
            "For pain relief, you may consider paracetamol/acetaminophen, or ibuprofen/naproxen only if these are safe for you. Avoid anti-inflammatory painkillers if you have stomach ulcers, kidney disease, blood thinners, pregnancy concerns, or an allergy—ask a doctor or pharmacist.",
        ...
        suggestions = [
            ...
            "For pain relief, use only medicines that are safe for you and follow the label or your clinician’s advice.",
            "Keep the uploaded image and this simple summary for your appointment.",
        ]
        ...
        with gr.Tabs(selected="welcome") as main_tabs:
                    ...
                    <div class="landing-lower">
                      ...
                      <div>
                        ...
                        <div class="why-grid">
                          ...
                          <div class="why-card"><div class="why-icon">⬇️</div><b>Downloadable PDF</b><span>Save your simplified summary for later.</span></div>
                        </div>
                      </div>
                    </div>
            ...
            with gr.Tab("Page 2 · Choose Option", id="choose"):
                ...
                <div class="choose-copy-v2">
                    ...
                    <p>Pick your report type — we’ll open the right page instantly<br>and guide you through every step.</p>
                    <div class="choose-info-v2">
                        <span>⌘</span>
                        <b>You’re one tap away.</b> The correct upload page opens the moment you choose.
                    </div>
                </div>
                """)
                ...
                with gr.Row(elem_classes=["choice-grid-v2"]):
                    ...
                    with gr.Column(elem_classes=["choice-card-v2 choice-xray-v2"]):
                        ...
                        <h3>X-ray Imaging</h3>
                        <p>Upload your X-ray and we’ll translate the findings into plain, reassuring language.</p>
                        """)
