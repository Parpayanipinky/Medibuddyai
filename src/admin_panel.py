...
def _empty_chart(title: str, icon: str = "ðŸ“Š", message: str = "No data available yet.") -> str:
    return f"""
...
def _bar_chart_html(title: str, values: Dict[str, int], icon: str = "ðŸ“Š", note: str = "") -> str:
    if not values:
...
def _line_chart_svg(title: str, values: List[float], suffix: str = "", max_y: float | None = None, icon: str = "ðŸ“ˆ") -> str:
    if not values:
        return _empty_chart(title, icon)
...
def _build_charts_html(rows: List[Dict[str, Any]]) -> str:
    ...
    <div class="section-block">
      <div class="section-header">
        <div>
          <div class="section-eyebrow">ðŸ“Š Analytics</div>
      ...
      <div class="chart-grid-2">
        {_bar_chart_html("Analysis Volume by Type", type_counts, "ðŸ§¬", "Lab and X-ray activity split.")}
        {_donut_chart_html("Risk Distribution", good_risk, warning_risk, danger_risk)}
      </div>

      <div class="chart-grid-3">
        {_line_chart_svg("OCR Quality Trend", ocr_values, "%", 100, "ðŸ”")}
        {_line_chart_svg("Risk Score Trend", risk_scores, "/100", 100, "âš¡")}
        {_line_chart_svg("Abnormal Findings Trend", abnormal_values, "", max(max(abnormal_values), 5) if abnormal_values else None, "ðŸ”¬")}
      </div>

      <div class="chart-grid-2">
        {_bar_chart_html("Top Report Subtypes", subtype_counts, "ðŸ“‹", "Most frequently analyzed report categories.")}
        {_bar_chart_html("Risk Level Breakdown", risk_counts, "ðŸš¨", "Counts for each logged risk label.")}
      </div>
    </div>
    """
...
def _metric_card(title: str, value: Any, caption: str, icon: str = "â—Ž", accent: str = "", sparkline_html: str = "") -> str:
    return f"""
...
def _build_dashboard_html() -> Tuple[str, pd.DataFrame]:
    ...
    # Status icon
    status_icon = {"good": "ðŸŸ¢", "warn": "ðŸŸ¡", "danger": "ðŸ”´", "idle": "âš«"}.get(health_class, "âš«")
    ...
    kpi_cards = "".join([
        _metric_card("Total Analyses", f"{total:,}", "All logged events", "â—Ž"),
        _metric_card("Lab Reports", f"{lab:,}", "Text / OCR report analyses", "ðŸ§ª"),
        _metric_card("X-Ray Reviews", f"{xray:,}", "Image-based analyses", "ðŸ©»"),
        _metric_card("High / Attention", f"{high:,}", "Cases requiring review", "âš ï¸", "kpi-danger"),
        _metric_card("Avg OCR Quality", f"{avg_ocr:.1f}%", "Mean extraction quality", "ðŸ”",
                     sparkline_html=_sparkline_svg(ocr_spark, "#38bdf8")),
        _metric_card("Avg Risk Score", f"{avg_risk_score:.1f}/100", "Rule-based risk estimate", "ðŸ“Š",
    ...
    <div class="dash-root">
      ...
      <div class="hero-banner">
        <div class="hero-left">
          <div class="hero-eyebrow">ðŸ¥ MediBuddy AI Â· Admin Dashboard</div>
      ...
      <div class="section-block">
        <div class="section-header">
          <div>
            <div class="section-eyebrow">âš¡ Key Metrics</div>
...
def handle_admin_login(password: str):
    if not _check_admin_password(password):
        return (
            ...
            "",
            _history_dataframe([]),
            "<div class='login-error'>âš ï¸ Incorrect password. Please try again.</div>",
        )
...
def refresh_admin_dashboard(is_logged_in: bool):
    ...
    dashboard_html, history_df = _build_dashboard_html()
    return dashboard_html, history_df, "<div class='status-ok'>âœ… Dashboard refreshed successfully.</div>"
...
def logout_admin():
    return (
        ...
        "",
        _history_dataframe([]),
        "<div class='status-ok'>ðŸ‘‹ Logged out successfully.</div>",
    )
...
ADMIN_CSS = """
/* â”€â”€ Design Tokens â€” MediBuddy Light Theme â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
:root {
  --bg-base:       #e8eef0;
  --bg-surface:    #dde5e8;
  --bg-card:       #ffffff;
...
/* â”€â”€ Dashboard Shell â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€ Section Blocks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€ Chart Grid Layouts â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
.chart-grid-2 {
  display: grid;
...
/* â”€â”€ Chart Cards â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
.chart-card {
  background: var(--bg-card);
  border: 1px solid var(--stroke);
  border-radius: var(--radius-lg);
...
/* â”€â”€ SVG Line Chart â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
.svg-wrap {
  width: 100%;
  border-radius: 12px;
  overflow: hidden;
...
/* â”€â”€ History Table â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
#history-table {
...
/* â”€â”€ Status indicators â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
with gr.Blocks(title="MediBuddy AI Admin", theme=MEDIBUDDY_THEME, css=ADMIN_CSS) as admin_demo:
    ...
    with gr.Group(visible=True, elem_id="login-shell") as login_view:
        ...
        login_btn  = gr.Button("Login to Dashboard â†’", variant="primary", elem_id="login-btn")
    ...
    with gr.Group(visible=False, elem_id="dashboard-group") as dashboard_view:
        with gr.Row(elem_id="top-controls"):
            refresh_btn = gr.Button("âŸ³  Refresh Dashboard", variant="primary", elem_id="refresh-btn")
        ...
        with gr.Tabs(elem_id="dash-tabs") as tabs:
            with gr.TabItem("ðŸ“Š Overview", id="overview"):
                admin_html = gr.HTML(elem_id="dashboard-shell")

            with gr.TabItem("ðŸ“‹ Analysis History", id="history"):
